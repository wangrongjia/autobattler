...
- 目标系统：Android API 36
- 架构：ARMv7、ARM64
- 输出位置：`outputs/ThreeKingdom-debug-v0.1.2.apk`
...
4. 点击“导出项目”。
5. 勾选调试导出，保存到 `outputs/ThreeKingdom-debug-v0.1.2.apk`。
...
```powershell
& 'C:\Users\admin\Downloads\Godot_v4.7-stable_win64.exe\Godot_v4.7-stable_win64_console.exe' --headless --path 'D:\github\autobattler' --export-debug 'Android' 'D:\github\autobattler\outputs\ThreeKingdom-debug-v0.1.2.apk'
```

手机通过 USB 连接并开启 USB 调试后，可以执行：

```powershell
& "$env:LOCALAPPDATA\Android\Sdk\platform-tools\adb.exe" install -r 'D:\github\autobattler\outputs\ThreeKingdom-debug-v0.1.2.apk'
```
