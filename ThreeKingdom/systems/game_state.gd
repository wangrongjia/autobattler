...
const HERO_COOLDOWN_DEFAULTS := {"liubei":6.0, "guanyu":6.3, "liushan":0.0, "zhangfei":6.6, "zhaoyun":5.7, "huangzhong":4.2, "machao":7.2, "zhugeliang":6.9, "jiangwei":4.5, "pangtong":5.4, "menghuo":5.7, "zhurong":5.1, "dailaidongzhu":4.9, "weiyan":5.4, "madai":21.0, "sunquan":10.0, "sunshangxiang":8.0, "taishici":4.0, "ganning":8.0, "huanggai":10.0, "caocao":5.6, "dianwei":5.2, "xuchu":6.4, "zhangliao":6.8, "yuejin":6.0, "xuhuang":5.6, "zhanghe":5.2, "yujin":4.8, "xiahouyuan":5.2, "caoren":5.2, "xiahoudun":5.2, "simayi":6.4, "guojia":6.4, "xunyu":6.4, "jiaxu":6.4, "lvbu":6.4, "dongzhuo":5.5, "diaochan":7.0, "chengong":0.0, "gaoshun":6.2, "yanliang":4.0, "wenchou":4.0, "gaolan":0.0, "qunzhanghe":5.6, "huatuo":6.0, "yuji":6.6, "zuoci":6.0, "zhangjiao":6.0, "zhangliang":5.0, "zhangbao":0.0}
...
var heroes := {
	...
	"lusu":{"zh":"鲁肃", "en":"Lu Su", "f":"wu", "hp":260, "skill_value":100, "cooldown":4.0, "range":2, "skill":"Alliance", "zh_skill":"连横稳阵", "ability":"signature", "summary":"治疗当前生命总量最低的友军并提高其最大生命。", "en_summary":"Treats the ally with the lowest current HP total and raises max HP."},
...
func _add_extended_roster() -> void:
	...
	_register_hero("zhanghe", "张郃", "Zhang He", "wei", 3780, 100, 5.2, 2, "Coiling Spear Chain", "巧变连枪", "signature", {"mult":4.0, "stun":1.5, "xuhuang_stun_bonus":1.0, "five_chain_targets":2, "five_damage_bonus_mult":2.0, "five_stunned_damage_bonus_mult":4.0}, "巧变连枪：随机攻击一名敌方前军，造成400%兵略值伤害并眩晕1.5秒。", "Coiling Spear Chain: Strike a random enemy vanguard for 400% Strategy damage and stun for 1.5s.")
	...
	_register_hero("xiahouyuan", "夏侯渊", "Xiahou Yuan", "wei", 2740, 100, 5.2, 3, "Swift Suppression", "神速震袭", "signature", {"target_count":2, "mult":2.20, "stun":1.0, "bond_cooldown_reduction":0.5, "bond_stun_bonus":0.5}, "神速震袭：随机攻击2名敌军，造成220%兵略值伤害并眩晕1秒。", "Swift Suppression: Strike 2 random enemies for 220% Strategy damage and stun for 1s.")
	_register_hero("caoren", "曹仁", "Cao Ren", "wei", 4120, 100, 5.2, 2, "Rearward Bulwark", "樊城镇远", "signature", {"target_count":2, "mult":2.0, "stun":1.0, "guard_time":5.0, "rear_reduction_skill_ratio":0.20, "bond_bonus_targets":1, "bond_stun_bonus":0.5, "bond_reduction_skill_ratio":0.10}, "樊城镇远：随机攻击2名敌方后军，造成200%兵略值伤害并眩晕1秒；释放后5秒内受到敌方后军的伤害减少0.2×兵略值%。", "Rearward Bulwark: Strike 2 enemy rearguards for 200% Strategy and stun for 1s; for 5s, take 0.2×Strategy% less damage from rearguards.")
	_register_hero("xiahoudun", "夏侯惇", "Xiahou Dun", "wei", 5900, 100, 5.2, 1, "Vanguard Bulwark", "刚烈镇前", "signature", {"target_count":2, "mult":2.40, "stun":1.5, "guard_time":5.0, "front_reduction_skill_ratio":0.20, "bond_bonus_targets":1, "bond_stun_bonus":0.5, "bond_reduction_skill_ratio":0.10}, "刚烈镇前：随机攻击2名敌方前军，造成240%兵略值伤害并眩晕1.5秒；释放后5秒内受到敌方前军的伤害减少0.2×兵略值%。", "Vanguard Bulwark: Strike 2 enemy vanguards for 240% Strategy and stun for 1.5s; for 5s, take 0.2×Strategy% less damage from vanguards.")
	...
	_register_hero("jiaxu", "贾诩", "Jia Xu", "wei", 2980, 100, 6.4, 3, "Venomous Scheme", "毒士奇谋", "signature", {"target_count":2, "duration":4.0, "poison_skill_ratio":0.02, "simayi_bonus_targets":1, "simayi_duration_penalty":1.0, "guojia_duration_bonus":1.0, "guojia_poison_bonus_skill_ratio":0.005, "xunyu_cooldown_reduction":1.6}, "毒士奇谋：使随机2名敌军中毒4秒，每秒损失0.02×兵略值%最大生命值。", "Venomous Scheme: Poison 2 random enemies for 4s, dealing 0.02×Strategy% max HP each second.")

	_register_hero("lvmeng", "吕蒙", "Lu Meng", "wu", 3680, 100, 2.8, 1, "White-Robed Raid", "白衣渡江", "strike", {"mult":2.1}, "突袭造成210%兵略值物理伤害；对远程目标额外提高30%。", "Ambush for 210% Strategy physical damage, +30% against ranged targets.")
	_register_hero("sunjian", "孙坚", "Sun Jian", "wu", 4120, 100, 3.0, 1, "Tiger's First Strike", "江东猛虎", "strike", {"mult":1.9}, "造成190%兵略值物理伤害；本场第一次释放提高至260%。", "Deal 190% Strategy physical damage; the first cast is increased to 260%.")
	_register_hero("sunce", "孙策", "Sun Ce", "wu", 5480, 100, 2.9, 1, "Little Conqueror", "小霸王", "drain", {"mult":1.55, "heal":0.40}, "造成155%兵略值物理伤害并回复实际伤害40%。生命越低，羁绊强化越高。", "Deal 155% Strategy physical damage and heal for 40%. Bonds scale as HP falls.")
	_register_hero("sunquan", "孙权", "Sun Quan", "wu", 285, 100, 10.0, 2, "Jiangdong Balance", "江东制衡", "signature", {"current_hp_damage_ratio":0.08, "max_hp_gain":200.0, "max_hp_cap_mult":2.0, "missing_hp_heal_ratio":0.10}, "随机对一名敌军造成其当前生命8%的伤害，提高自身最大生命并恢复已损生命。", "Damage a random enemy based on current HP, then grow max HP and restore missing HP.")
	_register_hero("sunshangxiang", "孙尚香", "Sun Shangxiang", "wu", 180, 100, 8.0, 3, "Heroine's Growing Volley", "枭姬叠势", "signature", {"hit_count":1, "mult":1.0, "skill_gain_per_cast":1.0, "ally_death_skill_gain":3.0}, "随机攻击一名敌军；每次施法和友军阵亡都会永久提高自身兵略值。", "Strike a random enemy; casts and allied deaths permanently increase Strategy.")
	_register_hero("daqiao", "大乔", "Da Qiao", "wu", 175, 100, 3.7, 3, "River Blossom", "国色流离", "heal", {"mult":1.5, "flat":95.0, "bond_missing_hp_step":0.10, "bond_heal_bonus_per_step":0.04}, "治疗当前生命比例最低的友军。与孙策组成江东佳偶后，目标每损失10%生命，本次受到的治疗提高4%。", "Heal the lowest-HP-ratio ally. With Sun Ce, the target gains 4% healing received per 10% HP missing.")
	_register_hero("xiaoqiao", "小乔", "Xiao Qiao", "wu", 165, 100, 3.5, 3, "Gentle Breeze", "天香缓阵", "signature", {"target_count":2, "slow":0.35, "slow_time":6.0}, "随机选择两名敌方后军，使其行动条速度降低35%，持续6秒。", "Slow two random enemy rearguards by 35% for 6s.")
	_register_hero("taishici", "太史慈", "Taishi Ci", "wu", 3600, 100, 4.0, 2, "Blazing Twin Halberds", "神亭烈戟", "signature", {"target_count":2, "mult":1.50, "burn":5.0, "burn_ratio":0.20, "sunce_target_count":3, "ganning_burning_mult":3.0}, "攻击射程内行动条最高的两名敌人，造成150%兵略值伤害并施加5秒灼烧。", "Strike the two reachable enemies with the highest gauges for 150% Strategy and burn them for 5s.")
	_register_hero("dingfeng", "丁奉", "Ding Feng", "wu", 335, 100, 8.5, 2, "Snowbound Short Blades", "雪中奋短兵", "signature", {"mult":1.30, "action_reduction":25.0, "bond_splash_mult":0.70, "bond_splash_action_reduction":15.0}, "攻击射程内行动条最高的敌人，造成130%兵略值物理伤害并压退25%行动条。", "Strike the reachable enemy with the highest gauge for 130% Strategy physical damage and push its gauge back by 25%.")
	_register_hero("xusheng", "徐盛", "Xu Sheng", "wu", 5380, 100, 3.2, 1, "Breaking Waves", "疑城水阵", "row_magic", {"mult":0.85, "stun":1.2}, "对随机敌排造成85%兵略值魔法伤害并减速1.2秒。", "Deal 85% Strategy magic damage to a row and disable it for 1.2s.")
	_register_hero("ganning", "甘宁", "Gan Ning", "wu", 3260, 100, 8.0, 2, "Bell-Raider Twin Assault", "锦帆并击", "signature", {"mult":1.50, "taishici_mult":2.50, "lvmeng_low_hp_bonus":0.50}, "自身与左侧友军分别攻击随机敌方后军，友军协击不消耗行动条。", "Gan Ning and his left ally each strike a random enemy rearguard; the ally assist costs no gauge.")
	_register_hero("huanggai", "黄盖", "Huang Gai", "wu", 4380, 100, 10.0, 1, "Bitter-Flesh Column", "苦肉焚阵", "signature", {"max_hp_cost":0.10, "damage_cost_ratio":0.33, "zhouyu_burn":6.0, "zhouyu_burn_cost_ratio":0.05, "sunjian_max_hp_cost":0.15, "sunjian_damage_cost_ratio":0.45}, "消耗10%最大生命，对随机敌方一列造成消耗生命33%的伤害。", "Spend 10% max HP to damage a random enemy column for 33% of HP spent.")
