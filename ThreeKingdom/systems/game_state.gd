...
var unit_inspector_scroll: ScrollContainer
var unit_inspector_unit_id := ""
var unit_inspector_preview: Dictionary = {}
var unit_inspector_restore_draft := false
...
var enemy_factions: Array[String] = []   # 闯关简单/一般/困难:本关敌方出没阵营(简单随机2/一般随机3/困难全4,随存档保存;王者地狱不走此限制)
...
var heroes := {
	...
	"guanyu":{"zh":"关羽", "en":"Guan Yu", "f":"shu", "hp":5180, "skill_value":100, "cooldown":6.3, "range":1, "skill":"Green Dragon Crescent", "zh_skill":"青龙偃月", "ability":"signature", "ability_params":{"mult":2.10, "five_mult":5.50, "peach_heal":0.30}, "summary":"青龙偃月：劈砍目标整列，每名敌人受到210%兵略值伤害。", "en_summary":"Cleave the target column for 210% Strategy damage to each enemy.", "detail_zh":"青龙偃月：劈砍目标整列，每名敌人受到210%兵略值伤害。", "detail_en":"Cleave the target column for 210% Strategy damage to each enemy."},
	"zhangfei":{"zh":"张飞", "en":"Zhang Fei", "f":"shu", "hp":4220, "skill_value":100, "cooldown":6.6, "range":2, "skill":"Command of Yan", "zh_skill":"燕人号令", "ability":"signature", "ability_params":{"damage_skill_ratio":0.15, "duration":3.3, "peach_duration_bonus":0.50, "five_duration_bonus":0.50, "five_damage_skill_ratio":0.25}, "summary":"燕人号令：强化己方前军，使其伤害提高0.15×兵略值%，持续3.3秒。", "en_summary":"Empower allied vanguards with a Strategy-scaled damage bonus for 3.3s.", "detail_zh":"燕人号令：强化己方前军，使其伤害提高0.15×兵略值%，持续3.3秒。", "detail_en":"Empower allied vanguards with a Strategy-scaled damage bonus for 3.3s."},
...
func _add_extended_roster() -> void:
	_register_hero("zhaoyun", "赵云", "Zhao Yun", "shu", 4200, 100, 5.7, 1, "Dragon-Gall Flurry", "龙胆连刺", "signature", {"hit_count":5, "hit_mult":1.15, "five_bonus_hits":1, "five_bonus_mult":1.0, "seven_bonus_hits":1}, "随机选择一名射程内敌人，快速攻击同一目标5次，每次造成115%兵略值伤害。", "Strike one random reachable enemy 5 times for 115% Strategy each.")
	_register_hero("huangzhong", "黄忠", "Huang Zhong", "shu", 3550, 100, 4.2, 3, "Piercing Arrow", "百步穿杨", "signature", {"mult":4.2, "five_mult":11.0, "meteor_crit_chance":0.30, "meteor_crit_mult":2.0}, "射击随机可攻击格，造成420%兵略值伤害。", "Shoot a random reachable tile for 420% Strategy damage.")
	...
	_register_hero("zhugeliang", "诸葛亮", "Zhuge Liang", "shu", 3460, 100, 6.9, 3, "Eight-Formation Stratagem", "八阵奇谋", "signature", {"mult":2.3, "menghuo_damage_bonus":0.20, "fire_mark_duration":15.0, "fire_mark_bonus":0.40, "liubei_extra_target_bonus":0.04}, "随机选择敌方格子，对目标及同列相邻格造成230%兵略值法术伤害。", "Deal 230% Strategy magic damage to a random tile and its vertical neighbors.")
	...
	_register_hero("zhurong", "祝融", "Zhurong", "shu", 3580, 100, 5.1, 3, "Flame Blade", "火神飞刃", "signature", {"mult":3.0, "burn":3.0, "burn_ratio":0.50, "bounce_mult":0.50, "sibling_burn_bonus":2.0, "sibling_burn_ratio":1.0}, "火神飞刃：对随机敌方单位造成300%兵略值法术伤害并灼烧3秒，每秒造成50%兵略值伤害。", "Flame Blade: Deal 300% Strategy magic damage to a random enemy and burn for 3s at 50% Strategy per second.")
	...
	_register_hero("madai", "马岱", "Ma Dai", "shu", 3580, 100, 21.0, 3, "General-Slaying Raid", "斩将突袭", "signature", {"max_hp_ratio":0.50, "empty_ruler_strategy_mult":20.0, "vulnerable_skill_ratio":0.30}, "随机攻击敌方前军，造成50%最大生命伤害；命中空格时对主公造成20×兵略值伤害。", "Strike a random enemy vanguard for 50% max HP; an empty tile deals 20x Strategy ruler damage.")
	_register_hero("pangtong", "庞统", "Pang Tong", "shu", 3550, 100, 5.4, 2, "Chain Scheme", "连环计", "signature", {"target_count":2, "mult":2.0, "link_duration":6.0, "link_ratio":0.30, "bond_target_count":3, "bond_link_ratio":0.50}, "随机攻击两个目标，各造成200%兵略值伤害并链接6秒；链接者受伤时其他目标承受30%同等伤害。", "Strike two random targets for 200% Strategy and link them for 6s; linked allies echo 30% damage.")

	_register_hero("zhangliao", "张辽", "Zhang Liao", "wei", 4180, 100, 6.8, 2, "Returning Blade", "威震回刃", "signature", {"mult":1.10, "hit_count":2, "yuejin_damage_bonus_mult":0.40, "five_damage_bonus_mult":1.00, "five_vulnerable_skill_ratio":0.40, "five_vulnerable_time":5.0}, "威震回刃：攻击随机敌方一列，回旋刃飞出与返回各造成110%兵略值伤害。", "Returning Blade: Strike a random enemy column for 110% Strategy on both the outward and returning passes.")
	_register_hero("yuejin", "乐进", "Yue Jin", "wei", 3420, 100, 6.0, 3, "Vanguard Volley", "先登乱射", "signature", {"target_count":3, "mult":2.0, "zhangliao_bonus_targets":1, "five_bonus_targets":1, "five_damage_bonus_mult":0.80, "five_grievous_skill_ratio":0.50, "five_grievous_time":5.0}, "先登乱射：随机攻击三名敌军，各造成200%兵略值伤害。", "Vanguard Volley: Strike three random enemies for 200% Strategy damage each.")
	...
	_register_hero("yujin", "于禁", "Yu Jin", "wei", 4970, 100, 4.8, 1, "Resolute Ward", "毅重护阵", "signature", {"target_count":1, "shield_mult":3.0, "five_bonus_targets":1, "five_shield_bonus_mult":2.0}, "毅重护阵：为当前生命值最低的友军施加300%兵略值的护盾。", "Resolute Ward: Shield the ally with the lowest current HP for 300% Strategy.")
	_register_hero("xiahouyuan", "夏侯渊", "Xiahou Yuan", "wei", 2740, 100, 5.2, 3, "Swift Suppression", "神速震袭", "signature", {"target_count":2, "mult":2.20, "stun":1.0, "bond_cooldown_reduction":0.5, "bond_stun_bonus":0.5, "bond_damage_bonus_mult":0.50}, "神速震袭：随机攻击2名敌军，造成220%兵略值伤害并眩晕1.8秒。", "Swift Suppression: Strike 2 random enemies for 220% Strategy damage and stun for 1.8s.")
	...
	_register_hero("guojia", "郭嘉", "Guo Jia", "wei", 2520, 100, 6.4, 3, "Frozen Legacy", "遗计冰封", "signature", {"target_count":2, "freeze":3.0, "empty_damage_mult":2.80, "shatter_per_second_mult":0.50, "simayi_bonus_targets":1, "simayi_duration_penalty":0.833333, "xunyu_duration_bonus":1.2, "jiaxu_cooldown_reduction":1.6}, "遗计冰封：随机冻结2名敌人5.4秒；空格造成280%兵略值伤害，冻结破碎时造成剩余秒数×50%兵略值伤害。", "Frozen Legacy: Freeze 2 random enemies for 5.4s; empty tiles take 280% Strategy damage and shattering deals 50% Strategy per remaining second.")
	...
	_register_hero("sunjian", "孙坚", "Sun Jian", "wu", 4120, 100, 15.0, 1, "Tiger's Resolve", "猛虎绝命", "signature", {"current_hp_cost":0.40, "damage_cost_ratio":1.0}, "猛虎绝命：消耗40%当前生命，攻击正前方敌军并造成等同于实际消耗生命100%的伤害。", "Tiger's Resolve: Spend 40% current HP and deal 100% of the HP actually spent to the facing enemy.")
	...
	_register_hero("chengong", "陈宫", "Chen Gong", "qun", 2980, 100, 0.0, 2, "Measured Formation", "智迟谋速", "passive", {"haste_ratio":0.24, "lvbu_haste_ratio":0.16, "gaoshun_haste_ratio":0.16}, "智迟谋速（被动）：陈宫及其同列友军获得相当于陈宫24%兵略值的冷却极速；与吕布/高顺同阵时各额外+16%兵略值。", "Measured Formation (Passive): Chen Gong and allies in his column gain cooldown haste equal to 24% of his Strategy (+16% each with Lu Bu / Gao Shun).")
...
func _apply_existing_faction_skill_reworks() -> void:
	...
	_set_skill("sunjian", {"current_hp_cost":0.40, "damage_cost_ratio":1.0, "sun_legacy_current_hp_cost":0.80, "sun_legacy_damage_skill_ratio":0.15}, "猛虎绝命：消耗40%当前生命，攻击正前方敌军并造成等同于实际消耗生命100%的伤害。", "Tiger's Resolve: Spend 40% current HP and deal 100% of the HP actually spent to the facing enemy.")
	heroes.sunce.skill = "Conqueror's Twin Assault"
	heroes.sunce.zh_skill = "小霸王连击"
	_set_skill("sunce", {"mult":1.60, "missing_hp_step":0.10, "missing_hp_damage_bonus_per_step":0.03, "sun_legacy_damage_bonus_mult":0.50, "taishici_bonus_mult":0.50, "missing_hp_reduction_per_step":0.04}, "小霸王连击：攻击正前方及其左侧敌军，各造成160%兵略值伤害；自身每损失10%生命，伤害提高3%。", "Conqueror's Twin Assault: Strike the facing enemy and its left neighbor for 160% Strategy each; gain 3% damage per 10% HP missing.")
	heroes.sunquan.skill = "Jiangdong Balance"
	heroes.sunquan.zh_skill = "江东制衡"
	_set_skill("sunquan", {"current_hp_damage_ratio":0.08, "max_hp_gain_skill_ratio":1.0, "max_hp_cap_mult":2.0, "missing_hp_heal_skill_ratio":0.10, "sun_legacy_max_hp_gain_skill_ratio":2.0, "sun_legacy_max_hp_cap_mult":3.0, "sun_legacy_missing_hp_heal_skill_ratio":0.15, "luxun_damage_ratio":0.11}, "江东制衡：提高100%兵略值最大生命并恢复0.1×兵略值%的已损生命，再造成当前生命8%的伤害；最大生命上限为初始值2倍。", "Jiangdong Balance: Gain max HP equal to 100% Strategy, restore Strategy-scaled missing HP, then deal 8% current HP damage, capped at twice initial max HP.")
	_set_skill("zhouyu", {"mult":2.0, "tile_count":2, "four_heroes_bonus_tiles":2, "burn":4.0, "burn_ratio":0.30, "four_heroes_burn_bonus_mult":0.30, "xiaoqiao_burn_duration_bonus":3.0, "xiaoqiao_burn_bonus_mult":0.30, "missing_hp_step":0.10, "missing_hp_bonus_per_step":0.06}, "赤壁点火：随机选择2个敌方格，各造成200%兵略值法术伤害并灼烧4秒，每秒造成30%兵略值伤害。", "Red Cliffs: Ignite 2 random enemy tiles for 200% Strategy magic damage and burn for 4s at 30% Strategy per second.")
	...
	_set_skill("lusu", {"heal_mult":3.20, "max_hp_skill_ratio":1.0, "target_count":1, "four_heroes_heal_mult":4.0, "four_heroes_max_hp_skill_ratio":2.0, "four_heroes_target_count":2}, "连横稳阵：为当前生命值最低的友军恢复320%兵略值生命，并使其最大生命提高100%兵略值。", "Alliance: Heal the ally with the lowest current HP for 320% Strategy and grant max HP equal to Strategy.")
	...
	_set_skill("sunshangxiang", {"target_count":1, "mult":5.0, "skill_gain_per_cast":1.0, "sun_legacy_release_count":2, "sun_legacy_skill_gain":2.0}, "枭姬叠势：随机攻击一名敌军，造成500%兵略值伤害，每次释放后兵略值提高1点。", "Heroine's Growing Volley: Strike one random enemy for 500% Strategy damage and gain 1 Strategy after casting.")
	...
	_set_skill("lvbu", {"mult":2.20, "dongzhuo_heal":0.20, "missing_hp_step":0.10, "diaochan_bonus_per_step":0.04, "chengong_repeat_chance":0.30}, "无双横扫：攻击正前方单元格和左右相邻单元格，各造成220%兵略值伤害。", "Peerless Sweep: Strike the facing tile and its left and right neighbors for 220% Strategy damage.")
	...
	_set_skill("diaochan", {"duration":4.0, "dongzhuo_duration_bonus":2.0, "dongzhuo_self_heal_mult":3.0, "forced_attack_interval":1.0, "forced_attack_mult":1.0, "target_mode":"random"}, "美人离间：随机魅惑一名敌军7.2秒，使其行动条停止。", "Beauty's Scheme: Charm a random enemy for 7.2s.")
	heroes.chengong.skill = "Measured Formation"
	heroes.chengong.zh_skill = "智迟谋速"
	heroes.chengong.ability = "passive"
	_set_skill("chengong", {"haste_ratio":0.24, "lvbu_haste_ratio":0.16, "gaoshun_haste_ratio":0.16}, "智迟谋速（被动）：陈宫及其同列友军获得相当于陈宫24%兵略值的冷却极速；与吕布/高顺同阵时各额外+16%兵略值。极速按公式叠加、收益递减。", "Measured Formation (Passive): Chen Gong and allies in his column gain cooldown haste equal to 24% of his Strategy (+16% each with Lu Bu / Gao Shun); haste stacks with diminishing returns.")
	...
	_set_skill("qunzhanghe", {"target_count":2, "shield_mult":2.0, "gaolan_shield_bonus_mult":0.60, "four_pillars_bonus_targets":2, "four_pillars_shield_bonus_mult":1.0}, "河北护阵：为当前生命值最低的两名友军施加可抵消200%兵略值伤害的护盾。", "Hebei Ward: Shield the 2 allies with the lowest current HP for 200% Strategy.")
	...
	_set_skill("yuji", {"target_count":2, "poison_stack_mult":1.40, "huatuo_bonus_targets":1, "fangshi_missing_hp_step":0.10, "fangshi_damage_bonus_per_step":0.05}, "蛊毒仙术：对随机两名敌军施加1.4×兵略值的中毒层数；每秒受到层数等量伤害，随后层数向下取整减半，降至0时清除。", "Venomous Immortal Art: Apply decaying poison stacks to 2 random enemies.")
	heroes.zuoci.skill = "Immortal Aid"
	heroes.zuoci.zh_skill = "遁甲济世"
	_set_skill("zuoci", {"target_count":2, "heal_mult":1.70, "huatuo_bonus_mult":1.00, "thunder_mult":2.0}, "遁甲济世：治疗当前生命值最低的两名友军，各恢复170%兵略值生命。", "Immortal Aid: Heal two allies for 170% Strategy each.")
	heroes.zhangjiao.skill = "Yellow Sky Thunder"
	heroes.zhangjiao.zh_skill = "黄天雷引"
	_set_skill("zhangjiao", {"target_count":2, "mult":3.0, "zhangliang_bonus_mult":1.20, "death_thunder_targets":2, "death_thunder_mult":6.0, "death_thunder_stun":1.111111}, "黄天雷引：召唤雷电随机攻击两名敌军，各造成300%兵略值伤害。", "Yellow Sky Thunder: Call lightning on two random enemies for 300% Strategy damage each.")
...
func _make_roster_unit(team: String, hero_id: String) -> Dictionary:
	...
	var unit := {
		...
		"freeze":0.0, "freeze_shatter_damage":0.0, "freeze_shatter_per_second":0.0, "freeze_source_id":"",
		...
		"bond_haste":0.0, "sunquan_initial_max_hp":0.0, "sunshangxiang_skill_bonus":0.0,
		...
		"skill_value_bonus":(float(enemy_strategy_bonus) if team == "enemy" else 0.0), "gaolan_skill_value_bonus":0.0, "gaolan_aura_source":"", "timed_skill_value_bonus":0.0, "timed_skill_value_time":0.0, "liushan_aura_damage_bonus":0.0, "liushan_aura_lifesteal":0.0, "chain_effects":[], "four_pillars":false,
