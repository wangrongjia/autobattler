...
const TALENT_TREES := {
	...
	"wei":{"name":"霸府经纬", "faction":"wei", "nodes":[
	...
	"qun":{"name":"乱世烽火", "faction":"qun", "nodes":[
...
func _draw_runes(count: int) -> Array:
	if count <= 0 or general_souls < count * 500: return []
	general_souls -= count * 500
	var results: Array = []
