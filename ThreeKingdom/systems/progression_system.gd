...
func _rune_effect(rune: Dictionary) -> Dictionary:
	...
	for stat in str(kind.get("positive", "")).split(",", false):
		...
		elif stat == "cooldown": result.cooldown += amount * 1.0   # 冷却极速点：1 点 = 1 极速（旧 0.05 秒 × 20 极速/秒）
...
func _talent_effect_description(tree_id: String, node_name: String) -> String:
	var specials := {
		...
		"all:群英":"所有武将生命 +400、兵略 +10、技能冷却极速 +10。",
	...
	if float(node[5]) != 0.0: effects.append("每级冷却极速 +%.0f，满级 +%.0f（实际冷却 = 原冷却 × 100/(100+极速)，收益递减）" % [float(node[5]) * 20.0, float(node[5]) * 20.0 * int(node[2])])
	return "；".join(effects) + "。"
...
func _talent_stat_bonus(hero_id: String) -> Dictionary:
	...
	for tree_id in ["all", faction]:
		for node in TALENT_TREES[tree_id].nodes:
			...
			result.cooldown += float(node[5]) * level * 20.0   # 天赋数据以秒计，×20 折算为冷却极速点
	return result
