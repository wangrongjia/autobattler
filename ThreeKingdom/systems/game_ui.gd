...
func _show_rune_hero_detail() -> void:
	...
	var total_haste := maxf(-90.0, talent_haste + float(runes.cooldown))
	var talent_cooldown := maxf(2.0, base_cooldown * 100.0 / (100.0 + maxf(-90.0, talent_haste)))
	...
	rune_hero_detail_columns.add_child(_rune_stat_panel("天赋加成", Color("#b98a4f"), roundi(talent_hp), roundi(talent_strategy), talent_cooldown, roundi(talent.hp), roundi(talent.strategy), talent_cooldown - base_cooldown))
	rune_hero_detail_columns.add_child(_rune_stat_panel("天赋 + 符文", Color("#c56a5a"), roundi(final_hp), roundi(final_strategy), final_cooldown, roundi(float(talent.hp) + rune_hp), roundi(float(talent.strategy) + float(runes.strategy)), final_cooldown - base_cooldown))
	...
	var data := {
		...
		"wei":{
			...
			"bonds":[
				...
				["swift_bulwark", "神速镇远", "Swift Bulwark", ["xiahouyuan", "caoren"], "2人", "2 heroes", "夏侯渊冷却极速+9、眩晕+0.9秒、伤害+50%；曹仁获得目标、眩晕与后军减伤强化。", "Xiahou Yuan gains 9 cooldown haste, stun, and damage; Cao Ren gains target, stun, and rear reduction."],
				["xiahou_brothers", "夏侯同心", "Xiahou Brothers", ["xiahouyuan", "xiahoudun"], "2人", "2 heroes", "夏侯渊冷却极速+9、眩晕+0.9秒、伤害+50%；夏侯惇获得目标、眩晕与前军减伤强化。", "Xiahou Yuan gains 9 cooldown haste, stun, and damage; Xiahou Dun gains target, stun, and vanguard reduction."],
				...
				["frost_venom", "冰毒奇策", "Frost and Venom", ["guojia", "jiaxu"], "2人", "2 heroes", "郭嘉冷却极速+22；贾诩目标+1、毒层数减少0.2×兵略值。", "Guo Jia gains 22 cooldown haste; Jia Xu gains 1 target but applies 0.2×Strategy fewer stacks."],
				["royal_venom", "王佐毒策", "Royal Venom", ["xunyu", "jiaxu"], "2人", "2 heroes", "荀彧与贾诩均获得22冷却极速。", "Xun Yu and Jia Xu each gain 22 cooldown haste."]
			]
		},
		"wu":{
			...
			"bonds":[
				...
				["jiangbiao_blades", "江表双锋", "Twin Blades of Jiangbiao", ["taishici", "ganning"], "2人", "2 heroes", "太史慈伤害增加60%兵略值；甘宁冷却极速+18。", "Taishi Ci gains 60% Strategy damage; Gan Ning gains 18 cooldown haste."],
		...
		"qun":{
			...
			"bonds":[
				...
				["peerless_strategy", "谋定无双", "Peerless Strategy", ["lvbu", "chengong"], "2人", "2 heroes", "吕布横扫有30%概率再释放一次；陈宫光环的极速系数额外+16%兵略值。", "Lu Bu has a 30% repeat chance; Chen Gong's aura gains another 16% of Strategy as haste."],
				...
				["strategy_formation", "谋陷并驱", "Strategy and Formation", ["chengong", "gaoshun"], "2人", "2 heroes", "陈宫光环的极速系数额外+16%兵略值；高顺易碎延长6.3秒。", "Chen Gong's aura gains another 16% of Strategy as haste; Gao Shun's Fragile gains 6.3s."],
...
func _skill_detail(hero_id: String) -> String:
	match hero_id:
		...
		"chengong": return t("智迟谋速（被动）：陈宫及其同列友军获得相当于陈宫24%兵略值的冷却极速；与吕布/高顺同阵时各额外+16%兵略值。极速按公式叠加、收益递减。", "Measured Formation (Passive): Chen Gong and allies in his column gain cooldown haste equal to 24% of his Strategy (+16% each with Lu Bu / Gao Shun); haste stacks with diminishing returns.")
...
func _hero_bond_detail(hero_id: String) -> String:
	...
	var combo_defs: Array = [
		...
		[["lvbu", "chengong"], "谋定无双", "Peerless Strategy", {"lvbu":["无双横扫有30%概率连续释放两次。", "Peerless Sweep has a 30% chance to cast twice."], "chengong":["智迟谋速的极速系数额外增加16%兵略值。", "Measured Formation gains another 16% of Strategy as cooldown haste."]}],
		...
		[["chengong", "gaoshun"], "谋陷并驱", "Strategy and Formation", {"chengong":["智迟谋速的极速系数额外增加16%兵略值。", "Measured Formation gains another 16% of Strategy as cooldown haste."], "gaoshun":["陷阵之志的易碎持续时间增加6.3秒。", "Formation Resolve Fragile duration gains 6.3s."]}],
		...
		[["xiahouyuan", "caoren"], "神速镇远", "Swift Bulwark", {"xiahouyuan":["冷却极速+9，眩晕延长0.9秒，伤害增加50%兵略值。", "Gain 9 cooldown haste, +0.9s stun, and +50% Strategy damage."], "caoren":["目标增加1名，眩晕延长0.9秒，释放技能后受到敌方后军伤害减免提高0.1*兵略值%。", "Gain 1 target, +0.9s stun, and +0.1*Strategy% rear damage reduction."]}],
		[["xiahouyuan", "xiahoudun"], "夏侯同心", "Xiahou Brothers", {"xiahouyuan":["冷却极速+9，眩晕延长0.9秒，伤害增加50%兵略值。", "Gain 9 cooldown haste, +0.9s stun, and +50% Strategy damage."], "xiahoudun":["目标增加1名，眩晕延长0.9秒，释放技能后受到敌方前军伤害减免提高0.1*兵略值%。", "Gain 1 target, +0.9s stun, and +0.1*Strategy% vanguard damage reduction."]}],
		...
		[["guojia", "jiaxu"], "冰毒奇策", "Frost and Venom", {"guojia":["冷却极速+22。", "Gain 22 cooldown haste."], "jiaxu":["施法目标增加1名，但施加的毒层数减少0.2×兵略值。", "Gain 1 poison target but apply 0.2×Strategy fewer stacks."]}],
		[["xunyu", "jiaxu"], "王佐毒策", "Royal Venom", {"xunyu":["冷却极速+22。", "Gain 22 cooldown haste."], "jiaxu":["冷却极速+22。", "Gain 22 cooldown haste."]}],
		...
		[["taishici", "ganning"], "江表双锋", "Twin Blades of Jiangbiao", {"taishici":["造成的伤害增加60%兵略值。", "Gain 60% Strategy damage."], "ganning":["冷却极速+18。", "Gain 18 cooldown haste."]}],
