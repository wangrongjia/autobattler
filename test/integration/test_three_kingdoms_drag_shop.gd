...
func _initialize() -> void:
	...
	assert(not game._can_drop_board(Vector2.ZERO, {"unit_id":second.id}, 0, 0))
	game._drop_reserve(Vector2.ZERO, {"unit_id":first.id}, 1)
	assert(game._find_by_id(game.player_units, first.id) != null)
	assert(first.row == -1 and first.col == -1)
	...
	assert(game._find_by_id(game.player_units, second.id) != null)
	assert(second.row == -1 and second.col == -1)
	quit()
